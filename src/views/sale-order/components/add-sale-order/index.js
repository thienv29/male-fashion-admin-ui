...
import { closeAddSaleOrder, setListSupplier, setLoading, setSupplierSelected } from '../../slice';
import {
    Divider,
    FormControl,
    Grid,
...
import SaleOrderService from '../../../../services/buy-order.service';
...
const AddSaleOrder = ({ saveCompleteEvent }) => {
    ...
    const handleClose = () => {
        dispatch(closeAddSaleOrder());
    };
    const handleAddSaleOrder = async (values) => {
        if (validateSaleOrder()) {
            const data = await SaleOrderService.create({ ...values, supplier: state.supplierSelected, listDetails });
    ...
    return (
        ...
        <Dialog open={state.addSaleOrder} onClose={handleClose} maxWidth={'md'}>
